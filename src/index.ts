...
export interface WsType {
  ...
  oneOfType<T extends PropTypes.Validator<any>>(
    types: T[]
  ): PropTypes.Requireable<NonNullable<PropTypes.InferType<T>>>;
  arrayOf<T>(type: PropTypes.Validator<T>): PropTypes.Requireable<T[]>;
  // // 强制规定对象 所有属性值都是指定的类型
  objectOf<T>(
    type: PropTypes.Validator<T>
  ): PropTypes.Requireable<{ [K in keyof any]: T }>;
  // null || {} 只要props满足Spec即可
  shape<P extends PropTypes.ValidationMap<any>>(
    type: P
  ): PropTypes.Requireable<PropTypes.InferProps<P>>;
  // null || {} 满足Spec 但不能有多余属性key-value
  exact<P extends PropTypes.ValidationMap<any>>(
    type: P
  ): PropTypes.Requireable<Required<PropTypes.InferProps<P>>>;
